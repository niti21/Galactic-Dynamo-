...
fig_path = "C:/Users/USER/desktop/Plasma/figures/Task-2/"
fig_save = True
```
...
def spatial_derivative(B, dx, order = 4, ghost_zone_type = 'symmetric'):
    '''
    B: 2D array for The magnetic field B.
    dx: The spatial step size.
    order: The order of the finite difference scheme. It can be 2 or 4. Default is 4.
    ghost_zone_type: str. The type of the ghost zone. It can be 'anti-symmetric', 'symmetric' or None. Default is 'symmetric'.
        
    Returns: The spatial-derivative of the magnetic field B and The second z-derivative of the magnetic field B.
    '''
...
```
    ...
    100%|██████████| 5000/5000 [00:25<00:00, 198.45it/s]
...
Nt = 5000  # Number of time steps
h = 1
order = 2 # order of the finite difference scheme
ghost_zone_type = 'symmetric' # type of the ghost zone
iterations_to_save = 'all' # which time steps to save
iterations_to_save_plot = [int(i * Nt/8) for i in range(8 + 1)] # which time steps to plot
...
```
    ...
    100%|██████████| 5000/5000 [00:09<00:00, 526.32it/s]
...
### Exploring Different Boundary conditions


```python
# simulating the time evolution of the magnetic field with different ghost zone types
B_list1 = sim_field(r_i, r_f, Nr, T, Nt, B_r01, B_phi01, dBrdt, dBphidt, order = order, ghost_zone_type = 'anti-symmetric', iterations_to_save = iterations_to_save)
B_list2 = sim_field(r_i, r_f, Nr, T, Nt, B_r01, B_phi01, dBrdt, dBphidt, order = order, ghost_zone_type = 'symmetric', iterations_to_save = iterations_to_save)
B_list3 = sim_field(r_i, r_f, Nr, T, Nt, B_r01, B_phi01, dBrdt, dBphidt, order = order, ghost_zone_type = 'relative anti-symmetric', iterations_to_save = iterations_to_save)
```

    100%|██████████| 5000/5000 [00:11<00:00, 435.88it/s]
    100%|██████████| 5000/5000 [00:21<00:00, 227.91it/s]
    100%|██████████| 5000/5000 [00:10<00:00, 480.92it/s]
...
if fig_save:
    plt.savefig(fig_path + 'Br_Bphi_evolution_t_anti-sym.png')
plt.show()
...
if fig_save:
    plt.savefig(fig_path + 'Br_Bphi_evolution_t_sym.png')
plt.show()
...
if fig_save:
    plt.savefig(fig_path + 'Br_Bphi_evolution_t_rel_anti-sym.png')
plt.show()
```
...
plt.xlabel('r')
plt.ylabel(r'$|B|$')
plt.title('Time evolution of $|B|$ - Anti-symmetric ghost zone')
plt.legend()
if fig_save:
    plt.savefig(fig_path + 'B_evolution_t_anti-sym.png')
plt.show()
...
plt.xlabel('r')
plt.ylabel(r'$|B|$')
plt.title('Time evolution of $|B|$ - Symmetric ghost zone')
plt.legend()
if fig_save:
    plt.savefig(fig_path + 'B_evolution_t_sym.png')
plt.show()
...
plt.xlabel('r')
plt.ylabel(r'$|B|$')
plt.title('Time evolution of $|B|$ - Relative anti-symmetric ghost zone')
plt.legend()
if fig_save:
    plt.savefig(fig_path + 'B_evolution_t_rel_anti-sym.png')
plt.show()
```
...
```
    ...
    100%|██████████| 60000/60000 [09:18<00:00, 107.43it/s] 
...
![png](output_26_0.png)
    ...
    saved figure at C:/Users/USER/desktop/Plasma/figures/Task-2/Br_Bphi_evolution_t_trial12.png
...
print(f'Dynamo numbers: {D[np.where(np.diff(np.sign(gamma)))[0]]}')
...
```
    ...
    Points where gamma crosses 0: [1.01502513]
    Dynamo numbers: [-3.31643523]
